<template>
  <NuxtLink :to="data.path" @click="setAttributes"><div :class="(store.getActiveMenu?.toLowerCase() == data.name.toLowerCase() || store.getHeaderValue?.toLowerCase() == data.name.toLowerCase()) ? 'sidebar-item active-nav' : 'sidebar-item'">
...
const setAttributes = () => {
  ...
  if(menuName.value)
  store.setActiveMenu(menuName.value)
  else
